...
const allArticles = async (req) => {
    ...
    try {
        const articles = await Article.find({ status: 'public' }).sort([[sortby, order]]).limit(parseInt(limit));
        return articles;
...
const moreArticles = async (req) => {
    ...
    try {
        const articles = await Article.find({ status: 'public' }).sort([[sortby, order]]).skip(skip).limit(parseInt(limit));
        return articles;
...
module.exports = { addArticle, getArticleById, getUsersArticleById, updateArticleById, deleteArticleById, allArticles, moreArticles, paginateAdminArticles }